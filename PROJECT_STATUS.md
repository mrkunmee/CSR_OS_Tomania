...
- ⬜ **3.4b Enforce isolation** ◀ **NEXT** — put `organizationId` in the session; scope **every** query + create + action by the caller's org; then a migration to make `organizationId` NOT NULL. (Until 3.4b lands, isolation is not yet enforced — single-org only.)
- ⬜ 3.4c Org onboarding (create org, invite users, org-scoped admin config UI).
...
**▶ Phase 3:** learning engine complete (3.1–3.3). Multi-agency **3.4a (schema foundation) done**; **next: 3.4b — enforce org isolation in every query/action + flip `organizationId` to NOT NULL.** (3.4b is the big query-scoping pass; until it lands the app is single-org.)
